

<section class="bg-gradient w-80 px-10 min-h-40vh d-flex ac jc flex-row" style="">
    <div class="d-flex as jc flex-row flex-wrap gap-5 max-w-1300px w-100">
        <div class="pt-20px d-flex flex-col ac jc font-family-roboto max-w-200px text-center h-100">
            <img src="img/fast.png" alt="">
            <span class="fw-light fs-4 my-10px">Fast</span>
            <span class="fw-regular fs-2">fast work, fast code, fast communication</span>
        </div>
        <div class="pt-20px d-flex flex-col ac jc font-family-roboto max-w-200px text-center">
            <img src="img/resposible.png" alt="">
            <span class="fw-light fs-4 my-10px">Resposive</span>
            <span class="fw-regular fs-2">all programs must work on small and big devices!</span>
        </div>
        <div class="pt-20px d-flex flex-col ac jc font-family-roboto max-w-200px text-center">
            <img src="img/intuitive.png" alt="">
            <span class="fw-light fs-4 my-10px">Intuitive</span>
            <span class="fw-regular fs-2">Strong preference for easy to use, intuitive UX/UI</span>
        </div>
        <div class="pt-20px d-flex flex-col ac jc font-family-roboto max-w-200px text-center">
            <img src="img/dynamic.png" alt="">
            <span class="fw-light fs-4 my-10px">Dynamic</span>
            <span class="fw-regular fs-2">fresh animated websites that looks wonderfull 🤩 </span>
        </div>
    </div>
</section>
<section class="w-80 px-10 d-flex ac jc flex-col">
    <div class="w-100 max-w-1000px d-flex flex-row flex-wrap jc ac reverse-row py-20px">
        <div class="w-50-dir fs-1 min-w-200px">
            <div class="py-10px">
                <span>HTML</span><span class="fw-light"> 90%</span>
                <div class="pasek">
                    <div class="loaded w-90"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>CSS</span><span class="fw-light"> 90%</span>
                <div class="pasek">
                    <div class="loaded w-90"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>JS</span><span class="fw-light"> 55%</span>
                <div class="pasek">
                    <div class="loaded w-55"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>PHP</span><span class="fw-light"> 60%</span>
                <div class="pasek">
                    <div class="loaded w-60"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>SketchUp</span><span class="fw-light"> 70%</span>
                <div class="pasek">
                    <div class="loaded w-70"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>Figma</span><span class="fw-light"> 50%</span>
                <div class="pasek">
                    <div class="loaded w-50"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>Premiere Pro</span><span class="fw-light"> 40%</span>
                <div class="pasek">
                    <div class="loaded w-40"></div>
                </div>
            </div>
        </div>
        <div class="w-50-dir text-center h-100 py-30px">
            <span>SOFTWARE</span>
        </div>
    </div>
    <div class="w-100 max-w-1000px d-flex flex-row flex-wrap jc ac py-20px">
        <div class="w-50-dir text-center h-100 py-30px">
            <span>HARDWARE</span>
        </div>
        <div class="w-50-dir fs-1 min-w-200px">
            <div class="py-10px">
                <span>Apple iPhone - service / rapair</span><span class="fw-light"> 70%</span>
                <div class="pasek">
                    <div class="loaded w-70"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>PC - build / service / repair / administration,</span><span class="fw-light"> 90%</span>
                <div class="pasek">
                    <div class="loaded w-90"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>Desktop builds and projects</span><span class="fw-light"> 80%</span>
                <div class="pasek">
                    <div class="loaded w-80"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>LED RGB, aRGB - using / implement / repair</span><span class="fw-light"> 90%</span>
                <div class="pasek">
                    <div class="loaded w-90"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>Smart home - web / devices</span><span class="fw-light"> 40%</span>
                <div class="pasek">
                    <div class="loaded w-40"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>Soldering medium stage</span><span class="fw-light"> 80%</span>
                <div class="pasek">
                    <div class="loaded w-80"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>Electronics - diagnosis / repair</span><span class="fw-light"> 60%</span>
                <div class="pasek">
                    <div class="loaded w-60"></div>
                </div>
            </div>
            <div class="py-10px">
                <span>Web and Servers</span><span class="fw-light"> 60%</span>
                <div class="pasek">
                    <div class="loaded w-60"></div>
                </div>
            </div>
        </div>

    </div>
</section>